...
class Solution
{
    ...
    private static Node toOutline(List<Heading> headings) {
        ...
        List<Heading> array = new ArrayList<Heading>();
        array.add(headings.get(0));
        array.add(headings.get(1));

        if(headings.size() > 2){
            for(int i = 2; i<headings.size(); i++){

                int weight = headings.get(i).weight;

                array.add(headings.get(i));

                if(weight==2){

                    root.children.add(new Node(headings.get(i)));
                }
                else if(weight==3){
                    for(int j = i; j > 0; --j){
                        if(array.get(j).weight==2){
                            String name = array.get(j).text;
                            for(Node child : root.children){
                                if(child.heading.text.equals(name)){
                                    child.children.add(new Node(headings.get(i)));
                                }
                            }
                            break;
                        }
